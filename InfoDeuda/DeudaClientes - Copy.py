...
recibosAyer = pd.read_sql(
 ''' 
    ...
    select SUM(Importe) 'Total Recibos Ayer',NROCLIENTE
    from RecVenta  
...
recibosAyerEfectivo = pd.read_sql(
 ''' 
    select SUM(r.Importe) as 'Total Recibos Negativos',NROCLIENTE
    from CCRec02 as r
  ...
  ,db_conex)
###########################################################################
##############################   Remitos  AYER Clientes ###################
###########################################################################
remitosAyer = pd.read_sql(
 ''' 

select sum(IMPORTE) as 'Remitos Ayer', NROCLIENTE
from FacRemDet
where FECHASQL < DATEADD(day, 0, CAST(GETDATE() AS date)) 
and FECHASQL >= DATEADD(day, -1, CAST(GETDATE() AS date))
and NROCLIENTE > 100000 and NROCLIENTE < 500000 
...
###########################################################################
################  Remitos SEMANALES ACUMULADOS Clientes ###################
###########################################################################
remitosSemanalesAcumulados = pd.read_sql(
 ''' 
select sum(IMPORTE) as 'Remitos Semanales Acumulados', NROCLIENTE
from FacRemDet
where FECHASQL < DATEADD(day, 0, CAST(GETDATE() AS date)) 
...
remitosSemanalesAcumulados = remitosSemanalesAcumulados.convert_dtypes()
...
deuda = deuda.merge(remitosSemanalesAcumulados,on='NROCLIENTE',how='left')
deuda['Remitos Semanales Acumulados'] = deuda['Remitos Semanales Acumulados'].fillna(0)
...
deuda['Dias de Venta Adeudado'] = deuda['Dias de Venta Adeudado'].fillna(1662)
deuda['Días Desde Última Compra'] = deuda['Días Desde Última Compra'].fillna(1662)
...
def calcular_interes(row):
    if row['Dias de Venta Adeudado'] > 15:
        ...
        dias_adeudados = row['Dias de Venta Adeudado'] - 15  # Días de venta adeudados que exceden los 15 días
...
pagosAyer = deuda.reindex(columns=['NROCLIENTE','NOMBRE','SALDOCUENTA','Remitos Semanales Acumulados','Remitos Ayer','Total Recibos Positivos','Total Recibos Negativos','Balance Diario','Tipo de deudor','Acuerdo','Vendedor'])
...
deuda1 = deuda.reindex(columns=['NROCLIENTE','NOMBRE','SALDOCUENTA','Cantidad Adeudada','Días Desde Última Compra','Acuerdo de Descubierto $','Dias de Venta Adeudado','Cupo Disponible','Interes por Mora','Vendedor','Tipo de deudor','Acuerdo'])
normal= deuda1.loc[deuda1["Tipo de deudor"] == "1-Normal",:]
moroso= deuda1.loc[deuda1["Tipo de deudor"] == "2-Moroso",:]
morosoG= deuda1.loc[deuda1["Tipo de deudor"] == "3-Moroso Grave",:]
gestionJ= deuda1.loc[deuda1["Tipo de deudor"] == "4-Gestión Judicial",:]
normal=normal.sort_values(['NOMBRE'])

moroso = moroso.sort_values(by="SALDOCUENTA", ascending=True)
morosoG = morosoG.sort_values(by="SALDOCUENTA", ascending=True)
gestionJ= gestionJ.sort_values(by="SALDOCUENTA", ascending=True)
...
deuda.loc["colTOTAL"]= pd.Series(
    ...
    , index=["SALDOCUENTA","Ventas Ultimos 15 Dias","Cantidad Adeudada",'Interes por Mora']
)
...
pagosAyer=pagosAyer.sort_values('SALDOCUENTA', ascending=True)
pagosAyer.loc["colTOTAL"]= pd.Series(
    pagosAyer.sum(numeric_only=True)
    , index=["SALDOCUENTA",'Remitos Semanales Acumulados','Remitos Ayer','Total Recibos Positivos','Total Recibos Negativos','Balance Diario']
)
...
ARGS:
    ...
    resultado = df.style \
        ...
        .apply(lambda x: ["color: white" if x.name == "colTOTAL" 
            ...
            , axis=1)\
        .applymap(
        lambda x: f'background-color: {color_map[x]}' if pd.notna(x) and x not in excluded_rows else '',
        subset=['Tipo de deudor']) \
        .applymap(
        lambda x: f'background-color: {color_map2[x]}' if pd.notna(x) and x not in excluded_rows else '',
        subset=['Acuerdo'])
    return resultado
...
numCols1=[]
numCols = [ 'NROCLIENTE','Días Desde Última Compra','Dias de Venta Adeudado','Cantidad Adeudada']
colpesos=[ 'SALDOCUENTA','Acuerdo de Descubierto $','Cupo Disponible','Interes por Mora']
coldeuda=['SALDOCUENTA']
colpesos1=[ 'SALDOCUENTA','Total Recibos Positivos', 'Remitos Semanales Acumulados','Remitos Ayer','Total Recibos Negativos','Balance Diario']
# Columnas Porcentaje
