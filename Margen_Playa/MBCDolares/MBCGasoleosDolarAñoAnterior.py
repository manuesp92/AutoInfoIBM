...
if fecha_actual.month == 1:
    inicio_mes_pasado = fecha_actual.replace(year=fecha_actual.year - 1, month=12, day=1)
else:
    inicio_mes_pasado = fecha_actual.replace(year=fecha_actual.year - 1, month=fecha_actual.month - 1, day=1)
...
# VARIABLES PARA MANIPULAR FECHAS
...
primer_dia_mes_actual = datetime.datetime(now.year, now.month, 1)  # Obtiene el primer día del mes actual
ultimo_dia_mes_anterior = primer_dia_mes_actual - timedelta(
    days=1)  # Resta un día para obtener el último día del mes anterior
primer_dia_mes_anterior = datetime.datetime(ultimo_dia_mes_anterior.year, ultimo_dia_mes_anterior.month,
                                   1)  # Obtiene el primer día del mes anterior
fecha_inicio_mes_anterior = primer_dia_mes_anterior.strftime('%Y-%m-%d')

hoy = datetime.datetime.now()
ayer = hoy - timedelta(days=1)
primer_dia_mes = ayer.replace(day=1)
pd.options.display.max_info_columns = 0  # Desactivar el resumen de columnas
pd.options.display.max_info_rows = 0  # Desactivar el resumen de filas
...
# comisionEUYearAnterior = comisionEU
# comisionGOYearAnterior = comisionGO

# comisionEUYearAnterior['Fecha'] = comisionEUYearAnterior['Fecha'] - timedelta(days=365)
# comisionGOYearAnterior['Fecha'] = comisionGOYearAnterior['Fecha'] - timedelta(days=365)

comisionEU['Fecha'] = comisionEU['Fecha'] - timedelta(days=365)
comisionGO['Fecha'] = comisionGO['Fecha'] - timedelta(days=365)


# comisionEU=pd.concat([comisionEU, comisionEUYearAnterior], ignore_index=True)
# comisionGO=pd.concat([comisionGO, comisionGOYearAnterior], ignore_index=True)

# comisionEU = comisionEUYearAnterior
# comisionGO = comisionGOYearAnterior
...
costoDapsaEU =costoDapsaEU.drop(columns = ['CODPRODUCTO'])
costoDapsaGO =costoDapsaGO.drop(columns = ['CODPRODUCTO'])

costoDapsa=costoDapsaEU.merge(costoDapsaGO, on = ['FECHASQL'], how = 'inner')
costoDapsa = costoDapsa.reindex(columns=['FECHASQL','COSTO GO DAP','COSTO EU DAP'])
costoDapsa.columns = ['FECHASQL', 'COSTO GO', 'COSTO EU']
...
mbcTotalGOAñoant['UEN'] = mbcTotalGOAñoant['UEN'].str.strip()
mbcTOTALEUAñoant['UEN'] = mbcTOTALEUAñoant['UEN'].str.strip()
