...
vtasSalonAM = pd.read_sql('''
	...
	SELECT A.UEN, COUNT(DISTINCT A.DESPACHOS) AS ID, SUM(A.Total) AS 'TOTAL VENDIDO Ant' FROM(
...
DECLARE @inicioMesAnterior DATETIME
SET @inicioMesAnterior = DATEADD(MONTH, -1, @inicioMesActual)

DECLARE @hoy DATETIME
SET @hoy = DATEADD(DAY, DATEDIFF(DAY, 0, CURRENT_TIMESTAMP), 0)

DECLARE @totalDiasMesAnterior INT
SET @totalDiasMesAnterior = DAY(EOMONTH(@inicioMesAnterior))

DECLARE @diasCursadosMesActual INT
SET @diasCursadosMesActual = DAY(CURRENT_TIMESTAMP) - 1

DECLARE @pondMesAct FLOAT
-- Si es el primer día del mes, se considera el total de días del mes anterior
IF @diasCursadosMesActual = 0
    SET @pondMesAct = 1
ELSE
    SET @pondMesAct = CAST(@totalDiasMesAnterior AS FLOAT) / @diasCursadosMesActual

SELECT A.UEN, COUNT(DISTINCT A.DESPACHOS) AS ID, SUM(A.Total) AS 'TOTAL VENDIDO'
FROM (
    SELECT ID AS DESPACHOS, UEN, (IMPORTE * @pondMesAct) AS Total
    FROM [Rumaos].[dbo].[SCEgreso]
...
vtasSalonAMT = vtasSalonAMT.assign(Intermensual= (vtasSalonAMT['TOTAL VENDIDO']/ vtasSalonAMT['TOTAL VENDIDO Ant'])-1)
...
tasa7 = (vtasSalonAMT.loc["colTOTAL","TOTAL VENDIDO"] /
 vtasSalonAMT.loc["colTOTAL","TOTAL VENDIDO Ant"]) -1
vtasSalonAMT.fillna({"Intermensual":tasa7}, inplace=True)




vtasSalonAMT = vtasSalonAMT.rename({"UEN":"UEN","Total Vendido Diario $":"Total Vendido Diario $","TOTAL VENDIDO Ant":"Total Proyectado Mes Anterior $"
,"Intermensual":"% Intermensual","TOTAL VENDIDO":"Total Proyectado Mes Actual $"}, axis=1)
