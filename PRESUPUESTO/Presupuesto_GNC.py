...
df_GNCpromos = pd.read_sql(''' 
 DECLARE @ayer DATETIME
	...
	DECLARE @hoy DATETIME
	SET @hoy = DATEADD(DAY, DATEDIFF(DAY, 0, CURRENT_TIMESTAMP), 0)
	SELECT   emp.UEN,sum(emp.VOLUMEN) as 'volumen promo',emp.FECHASQL
...
df_gncYPF['Cantidad Vendida PC'] = (df_gncYPF['Ventas Efectivo']+df_gncYPF['Venta Cta Cte']+df_gncYPF['ventas Promos pc']-df_gncYPF['volumen promo'])
...
def mbcypf(df, estacion):
    ...
    df = df.reindex(columns=['UEN', 'Cantidad Vendida PC', 'Venta diaria'])
    df = df.groupby(['UEN'],as_index=False).sum(numeric_only=True)
    print(df)
    return df
...
print(perdriel1)
mbcTOTAL=perdriel1.merge(perdriel2,on=['UEN','Cantidad Vendida PC','Venta diaria'],how='outer')
mbcTOTAL=mbcTOTAL.merge(azcuenaga,on=['UEN','Cantidad Vendida PC','Venta diaria'],how='outer')
mbcTOTAL=mbcTOTAL.merge(san_Jose,on=['UEN','Cantidad Vendida PC','Venta diaria'],how='outer')
mbcTOTAL=mbcTOTAL.merge(puente_Olive,on=['UEN','Cantidad Vendida PC','Venta diaria'],how='outer')
mbcTOTAL=mbcTOTAL.merge(las_heras,on=['UEN','Cantidad Vendida PC','Venta diaria'],how='outer')
mbcTOTAL=mbcTOTAL.merge(lamadrid,on=['UEN','Cantidad Vendida PC','Venta diaria'],how='outer')
mbcTOTAL=mbcTOTAL.merge(mercado1,on=['UEN','Cantidad Vendida PC','Venta diaria'],how='outer')
mbcTOTAL=mbcTOTAL.merge(mercado2,on=['UEN','Cantidad Vendida PC','Venta diaria'],how='outer')
mbcTOTAL=mbcTOTAL.merge(sarmiento,on=['UEN','Cantidad Vendida PC','Venta diaria'],how='outer')
mbcTOTAL=mbcTOTAL.merge(villa_nueva,on=['UEN','Cantidad Vendida PC','Venta diaria'],how='outer')
mbcTOTAL=mbcTOTAL.merge(adolfo_Calle,on=['UEN','Cantidad Vendida PC','Venta diaria'],how='outer')
mbcTOTAL=mbcTOTAL.merge(mitre,on=['UEN','Cantidad Vendida PC','Venta diaria'],how='outer')
mbcTOTAL=mbcTOTAL.merge(urquiza,on=['UEN','Cantidad Vendida PC','Venta diaria'],how='outer')
...
mbcTOTAL.loc["colTOTAL"]= pd.Series(
    ...
    , index=['Volumen Total Vendido','Presupuesto Acumulado L']
)
...
mbcTOTAL=mbcTOTAL.reindex(columns=['UEN','Presupuesto Acumulado L','Volumen Total Vendido','Desvio Presupuestado L','Desvio Presupuestado L %'])
mbcTOTAL= mbcTOTAL.rename({'Presupuesto Acumulado L':'Presupuesto Acumulado m3', 'Desvio Presupuestado L %':'Desvio Presupuestado m3 %', 'Desvio Presupuestado L':'Desvio Presupuestado m3'}, axis=1)
...
ARGS:
    ...
    resultado = df.style \
        ...
        .format("{:,.2f} m3", subset=list_Col_litros) \
        .hide(axis=0) \
        .set_caption(
            titulo
            + "<br>"
            ...
            + "<br>") \
        .applymap(lambda x: 'color: blue' if x >= 0 else 'color: #FF0000', subset=pd.IndexSlice[:, ['Desvio Presupuestado m3 %']]) \
        .applymap(lambda x: 'color: blue' if x >= 0 else 'color: #FF0000', subset=pd.IndexSlice[:, ['Desvio Presupuestado m3']]) \
...
EUmColslitros=['Presupuesto Acumulado m3','Volumen Total Vendido','Desvio Presupuestado m3']
# Columnas Porcentaje
percColsPen = ['Desvio Presupuestado m3 %']

### APLICO EL FORMATO A LA TABLA
mbcTOTAL = _estiladorVtaTituloD(mbcTOTAL,EUmColslitros,percColsPen, "Ejecucion Presupuestaria GNC")
##############
##############
