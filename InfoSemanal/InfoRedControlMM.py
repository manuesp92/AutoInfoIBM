...
def pre_RedControlSemanal(conexMSSQL):
    ...
    df_rem_sem_ant = pd.read_sql(
        ...
        SELECT 
            ...
            ,RTRIM(FPU.[CODPRODPLAYA]) AS 'CODPRODUCTO'
            ,FRD.[NROCLIENTE]
        ...
        LEFT JOIN Rumaos.dbo.FacProdUEN AS FPU ON FRD.CODPRODUCTO = FPU.CODPRODUCTO AND FRD.UEN = FPU.UEN
        WHERE FRD.FECHASQL >= @inicioSemanaAnterior
            AND FRD.FECHASQL < @inicioSemanaActual
            AND FRD.NROCLIENTE >= '100000' AND FRD.NROCLIENTE < '500000'
            AND FRD.CANTIDAD > 0
            AND FRD.UEN IN (
                'MERC GUAYMALLEN'
                ,'MERCADO 2'
            ...
            AND FPU.CODPRODPLAYA IN ('GO','EU','NS','NU')
        """
            ,conexMSSQL)
    ...
    df_rem_sem_act = pd.read_sql(
        ...
        WHERE FRD.FECHASQL >= @inicioSemanaActual
            ...
            AND FPU.CODPRODPLAYA IN ('GO','EU','NU','NS')
        """
            ,conexMSSQL)
...
DECLARE @ayer DATETIME
    ...
    -- Calcula la cantidad de días transcurridos en el mes actual
    DECLARE @diasTranscurridos INT
    SET @diasTranscurridos = DATEDIFF(DAY, @inicioMesActual, @hoy)

        SELECT 
            RTRIM(FRD.[UEN]) AS 'UEN'
            ,RTRIM(FPU.[CODPRODUCTO]) AS 'CODPRODUCTO'
            ,FRD.[NROCLIENTE]
            ,(FRD.[CANTIDAD] / @diasTranscurridos * DAY(EOMONTH(@ayer))) AS 'Mes Actual' 
            ,(FC.SALDOPREPAGO - FC.SALDOREMIPENDFACTU) as 'Saldo_Cta'
    ...
    def _preparador(df_ant, df_act, cycle, balance):
        ...
        # Convertir la columna "UEN" a tipo de datos string y rellenamos el valor na en la columna UEN con el string TOTAL
        #df_merge["UEN"] = df_merge["UEN"].astype(str)
