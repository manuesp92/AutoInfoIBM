...
df_PuenteMed_Ayer['Puente Medicion'] = df_PuenteMed_Ayer['Puente Medicion'].str.replace('.', '').str.replace(',', '.').astype(float)
print(df_PuenteMed_Ayer)
# Ventas GNC Sges
...
df_VtasGnc_Mes['Ventas Acumuladas SGES'] = df_VtasGnc_Mes.groupby('UEN')['VTATOTVOL'].cumsum()
df_VtasGnc_Mes = df_VtasGnc_Mes.loc[((df_VtasGnc_Mes["Fecha"] == ayer.strftime('%Y-%m-%d')) & (df_VtasGnc_Mes["Turno"] == 1))]


df_VtasGnc_Mes = df_VtasGnc_Mes.loc[((df_VtasGnc_Mes["Fecha"] == ayer.strftime('%Y-%m-%d')) & (df_VtasGnc_Mes["Turno"] == 1))]
df_VtasGnc_Mes=df_VtasGnc_Mes.reindex(columns=['UEN','Ventas Acumuladas SGES'])

# Ventas GNC Puente de Medicion
df_PuenteMed_Mes = df_PuenteMed.loc[(df_PuenteMed["Fecha"] <= (ayer- timedelta(days=1)).strftime('%Y-%m-%d'))]
df_PuenteMed_Mes['Puente Medicion'] = df_PuenteMed_Mes['Puente Medicion'].str.replace('.', '').str.replace(',', '.').astype(float)
...
df_Calibracion_Mes['CALIBRACION'] = (df_Calibracion_Mes['Ventas Acumuladas SGES']/df_Calibracion_Mes['Ventas Acumuladas'])-1
...
def _estiladorVtaTituloPAyer(df,list_Col_Num, list_Col_Num0,list_Col_Perc, titulo, evitarTotal):
    """
This function will return a styled dataframe that must be assign to a variable.
ARGS:
    df: Dataframe that will be styled.
...
alerta_Mes = _estiladorVtaTituloP(df_Calibracion_Mes, numCols, numCols0, percColsPen, 'Mermas GNC Acumulado',0)


alerta_Ayer = _estiladorVtaTituloPAyer(df_consolidado_Ayer, numCols, numCols0, percColsPen, 'INFO Calibracion GNC Ayer',0)
